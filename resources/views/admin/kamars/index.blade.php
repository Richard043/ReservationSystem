...
<body>
    ...
    <div class="content">
        <div class="card">
            <h1 class="text-center mb-4">Manage Kamars</h1>
            <a href="{{ route('admin.kamars.create') }}" class="btn btn-primary mb-3">Add New Kamar</a>
            @if(session('success'))
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $kamar->id }}</td>
                        <td style="font-weight: bold;">{{ $kamar->nomor_kamar }}</td>
                        <td>{{ $kamar->tipe_kamar }}</td>
                        <td>{{ number_format($kamar->harga, 2) }}</td>
                        <td>{{ $kamar->hotel->nama_hotel }}</td>
                        <td>
                            <a href="{{ route('admin.kamars.show', $kamar->id) }}" class="btn btn-info btn-sm">View</a>
                            <a href="{{ route('admin.kamars.edit', $kamar->id) }}" class="btn btn-warning btn-sm">Edit</a>
                            <form action="{{ route('admin.kamars.destroy', $kamar->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
