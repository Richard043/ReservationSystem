...
<body>
    ...
    <div class="content">
        <div class="card">
            <h1 class="text-center mb-4">Manage Tamus</h1>
            <a href="{{ route('admin.tamus.create') }}" class="btn btn-primary mb-3">Add New Tamu</a>
            @if(session('success'))
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $tamu->id }}</td>
                        <td style="font-weight: bold;">{{ $tamu->nama }}</td>
                        <td>{{ $tamu->email }}</td>
                        <td>{{ $tamu->telepon }}</td>
                        <td>
                            <a href="{{ route('admin.tamus.show', $tamu->id) }}" class="btn btn-info btn-sm">View</a>
                            <a href="{{ route('admin.tamus.edit', $tamu->id) }}" class="btn btn-warning btn-sm">Edit</a>
                            <form action="{{ route('admin.tamus.destroy', $tamu->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
